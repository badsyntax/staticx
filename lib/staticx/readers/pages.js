...
pages.read = function(dir, done) {
  ...
  var parser = parseMarkdownFile.bind(null, dir);

  fs.exists(dir, function(exists) {
    if (!exists) return done('Source directory does not exist: ' + dir);
    async.waterfall([
      glob.bind(glob, '_source/pages/**/*.md', {
      ...
      done(err, !err ? pages.sort(function(a, b) {
        return (parseInt(b.order, 10) || Infinity) - (parseInt(a.order, 10) || Infinity);
      }) : null);
    });
  });
};
