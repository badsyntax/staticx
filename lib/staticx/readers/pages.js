...
function readMarkdownFile(dir, file, done) {
...
function readMarkdownFiles(dir, done, err, files) {
  if (err) throw err;
  async.map(files, readMarkdownFile.bind(null, dir), function(err, data) {
    if (err) throw err;
    done(null, data);
  });
}
...
pages.read = function(dir, done) {
  fs.exists(dir, function(exists) {
    ...
    glob("**/*.md", globOpts, readMarkdownFiles.bind(null, dir, done));
  });
};
