...
Validator.implement('file-exists', function (options) {
  if (!fs.existsSync(options.data)) {
    options.error('File does not exist: ' + options.data);
  }
});

/**
 * Check if the specified parent page exists on the filesystem.
 * @param  {Object} options The validation options.
 */
Validator.implement('parent-exists', function (options) {
  ...
  if (!fs.existsSync(filePath)) {
    options.error('Parent page does not exist: ' + options.data);
  }
});

/**
 * A helper method to convert the validator error object into a simpler object to
 * work with.
