...
scaffold.copy = function(source, destination, done) {
  ...
  if (!fs.existsSync(source)) {
    return done('Scaffold copy: Source path must exist.');
  }
  if (!fs.existsSync(destination)) {
    return done('Scaffold copy: Destination path must exist.');
  }

   // Remove unwanted files from the source directory.
  ...
  fs.copy(source, destination, function filter(file) {
    return !regexFilter.test(file);
  }, done);
};
