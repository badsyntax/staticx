...
scaffold.copy = function(source, destination, done) {
  async.forEach([source, destination], function(dir, callback) {
...
scaffold.create = (function(options, done) {
  ...
  function init(opt, next) {
    ...
    options = _.merge({ source: 'lib/skeleton' }, opt);
...
scaffold.addPage = function(options, done) {
  ...
  if (!validate.valid) {
    return done(util.format('Error adding the page. %s: %s', validate.errors[0].property, validate.errors[0].message));
  }
  page.save(done);
};
